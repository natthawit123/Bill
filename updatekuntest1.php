...
$projact_bill = $_GET["projact_bill"];
                   ...
                   while ($showmember1 = mysqli_fetch_assoc($qresult1)) {
                    ...
                    <td class='col-md-2'>$showmember1[vat_amount]</td>
                    ...
                    <br/>";
                        $sql = "UPDATE `add` SET `total_amount`='$total_amount' WHERE id=$showmember1[id]";
                        $result = mysqli_query($con, $sql) or die ("Error in query: $sql " . mysqli_error($sql));
                        }
                        if ($result){
                            echo "<script type='text/javascript'>";
                                echo "window.location='updatetestsum.php?projact_bill=$projact_bill'";
                            echo "</script>";
                            }else{
                                echo "<script type='text/javascript'>";
                                    echo "alert('Error');";
